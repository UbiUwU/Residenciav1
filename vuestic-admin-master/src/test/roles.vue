...
const actualizarRol = async () => {
    try {
        ...
        const res = await api.actualizarRol(
            rolEditando.value.idrol,
            { Nombre: rolEditando.value.nombre }
        );
...
<template>
    <div class="p-4">
        <va-card>
            ...
            <va-card-content>
                <div class="flex flex-wrap gap-4 items-end">
                    <va-input v-model="nuevoRol.idrol" type="number" label="ID del Rol" class="w-48"
                        :rules="[value => !!value || 'El ID es requerido']" />
                    <va-input v-model="nuevoRol.nombre" label="Nombre del Rol" class="flex-1"
        ...
        <va-card class="mt-6">
            ...
            <va-card-content>
                <va-data-table :items="roles" :columns="[
                    { key: 'idrol', label: 'ID' },
        ...
        <va-modal v-model="mostrarModalEdicion" title="Editar Rol" hide-default-actions size="small">
            <va-form @submit.prevent="actualizarRol">
                <va-input v-model="rolEditando.nombre" label="Nombre del Rol" class="mb-4"
                    :rules="[value => !!value || 'El nombre es requerido']" />
                <div class="flex justify-end gap-2">
                    <va-button type="button" color="secondary" @click="mostrarModalEdicion = false">
                        Cancelar
                    </va-button>
        ...
        <va-modal v-model="mostrarModalConfirmacion" title="Confirmar Eliminación" hide-default-actions size="small">
            <template v-if="rolAEliminar">
                <p>¿Estás seguro de eliminar el rol <strong>"{{ rolAEliminar.nombre }}"</strong> (ID: {{
                    rolAEliminar.idrol }})?</p>
                <small class="text-warning">Esta acción no se puede deshacer</small>
            </template>
            <div class="flex justify-end gap-2 mt-4">
                <va-button color="secondary" @click="mostrarModalConfirmacion = false">
                    Cancelar
                </va-button>
                <va-button color="danger" @click="eliminarRol">
                    Eliminar
                </va-button>
            </div>
        </va-modal>
