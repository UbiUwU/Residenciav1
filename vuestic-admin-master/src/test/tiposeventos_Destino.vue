    ...
    <div class="grid gap-6">
        ...
        <va-card>
            <va-card-title>
                ...
                <va-button color="primary" icon="add" @click="mostrarModalCrearTipoEvento">
                    Nuevo Tipo
                </va-button>
            </va-card-title>
            ...
            <va-card-content>
                <va-data-table :items="tiposEvento" :columns="columnasTipoEvento" :loading="cargandoTipos">
                    <template #cell(actions)="{ row }">
                        <va-button size="small" color="info" icon="edit" class="mr-2"
                            @click="mostrarModalEditarTipoEvento(row)" />
                        <va-button size="small" color="danger" icon="delete"
                            @click="confirmarEliminarTipoEvento(row)" />
                    </template>
                </va-data-table>

                <!-- Modal para Tipo Evento -->
                <va-modal v-model="mostrarModalTipoEvento" :title="modalTituloTipoEvento" size="small"
                    hide-default-actions>
                    <va-form @submit.prevent="guardarTipoEvento">
                        <va-input v-model="formTipoEvento.nombre" label="Nombre" class="mb-4"
                            :rules="[v => !!v || 'Campo requerido']" />

                        <va-input v-model="formTipoEvento.descripcion" label="Descripción" class="mb-4"
                            type="textarea" />

                        <div class="flex justify-end gap-2 mt-4">
                            <va-button type="button" color="secondary" @click="mostrarModalTipoEvento = false">
                                Cancelar
                            </va-button>
        ...
        <va-card>
            <va-card-title>
                ...
                <va-button color="primary" icon="add" @click="mostrarModalCrearPublicoDestino">
                    Nuevo Público
                </va-button>
            </va-card-title>
            ...
            <va-card-content>
                <va-data-table :items="publicosDestino" :columns="columnasPublicoDestino" :loading="cargandoPublicos">
                    <template #cell(actions)="{ row }">
                        ...
                        <va-button size="small" color="danger" icon="delete"
                            @click="confirmarEliminarPublicoDestino(row)" />
                    </template>
                </va-data-table>

                <!-- Modal para Público Destino -->
                <va-modal v-model="mostrarModalPublicoDestino" :title="modalTituloPublicoDestino" size="small"
                    hide-default-actions>
                    <va-form @submit.prevent="guardarPublicoDestino">
                        <va-input v-model="formPublicoDestino.nombre" label="Nombre" class="mb-4"
                            :rules="[v => !!v || 'Campo requerido']" />
