<template>
  <VaCard>
    <VaCardTitle>Horario Semestral</VaCardTitle>
    <VaCardContent>
      <!-- Selector de semanas -->
      <div class="flex justify-between mb-4">
        <VaButton icon="chevron_left" :disabled="semanaActual === 1" @click="semanaAnterior" />
        <h3 class="text-center">Semana {{ semanaActual }}</h3>
        <VaButton icon="chevron_right" :disabled="semanaActual === totalSemanas" @click="semanaSiguiente" />
      </div>
...
export default {
  setup() {
    ...
    const toggleHora = (dia, hora, semana) => {
      ...
      const index = horarioSeleccionado.value.findIndex((h) => h.dia === dia && h.hora === hora && h.semana === semana)
    ...
    const isHoraSeleccionada = (dia, hora, semana) => {
      return horarioSeleccionado.value.some((h) => h.dia === dia && h.hora === hora && h.semana === semana)
    }
    ...
    const isHoraOcupada = (dia, hora, semana) => {
      ...
      return horarios.value.some(
        (h) => h.semana === semana && h.dia_semana === dia && h.hora_inicio <= horaStr && h.hora_fin >= horaFinStr,
      )
    }
    ...
    const isHoraDisponible = (dia, hora, semana) => {
      ...
      const horario = horarios.value.find(
        (h) => h.semana === semana && h.dia_semana === dia && h.hora_inicio <= horaStr && h.hora_fin >= horaFinStr,
      )
