...
<template>
  <div class="p-6">
    ...
    <div class="mb-6 border p-4 rounded-lg shadow">
      ...
      <form class="grid gap-2" @submit.prevent="guardarPlantilla">
        ...
        <input
          v-model="form.periodo_escolar_id"
          type="number"
          placeholder="Periodo Escolar ID"
          class="border p-2 rounded"
        />
        <input type="file" class="border p-2 rounded" @change="(e) => (form.archivo = e.target.files[0])" />

        <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
          {{ modoEdicion ? 'Actualizar' : 'Crear' }}
        </button>
        <button v-if="modoEdicion" type="button" class="bg-gray-500 text-white px-4 py-2 rounded" @click="resetForm">
          Cancelar
        </button>
      </form>
    ...
    <div v-else class="grid gap-4">
      <div v-for="p in plantillas" :key="p.id" class="p-4 border rounded shadow flex justify-between items-center">
        <div>
          <h3 class="font-bold">{{ p.nombre }}</h3>
        ...
        <div class="flex gap-2">
          <button class="px-3 py-1 bg-yellow-500 text-white rounded" @click="editarPlantilla(p)">Editar</button>
          <button
            class="px-3 py-1 bg-indigo-600 text-white rounded"
            @click="cambiarEstado(p, p.estado_id === 1 ? 2 : 1)"
          >
            {{ p.estado_id === 1 ? 'Desactivar' : 'Activar' }}
          </button>
          <button class="px-3 py-1 bg-red-600 text-white rounded" @click="eliminarPlantilla(p.id)">Eliminar</button>
        </div>
      </div>
    </div>
